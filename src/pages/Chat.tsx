      ...
      <div className="flex-1 flex flex-col p-6">
        <div className="flex-1 flex flex-col items-center justify-center max-w-4xl mx-auto w-full">
          <div className="text-center space-y-4 mb-8">
            ...
            <h1 className="text-3xl font-bold">Hi! I'm your AI social media posting assistant.</h1>
            <p className="text-lg text-muted-foreground">Upload files, ask questions, or tell me what you'd like to post. I'll help you create engaging content.
You dont even have to put a caption, but it will help if you put the nae of the item.</p>
          ...
          <div className="w-full space-y-3">
            <div className="flex space-x-4">
              <Textarea placeholder="Write whatever you would like the caption to talk about (e.g., 'This is the Godfather Sandwich' or 'write about a bogo promo tomorrow from 2-4pm')" value={message} onChange={e => setMessage(e.target.value)} className="flex-1 min-h-[120px] resize-none rounded-2xl bg-background/95 border border-border shadow-sm focus-visible:ring-ring" onKeyDown={e => {
            ...
            <div className="flex items-center space-x-2">
              <input type="file" multiple accept="image/*,video/*,.zip,.csv" onChange={handleFileUpload} className="hidden" id="chat-file-upload" />
              <Button variant="secondary" size="sm" className="rounded-full shadow-sm" onClick={() => document.getElementById('chat-file-upload')?.click()} disabled={isLoading}>
            ...
            <div className="flex justify-between items-center">
              ...
              <Button onClick={handleSend} disabled={!message.trim() && uploadedAssets.length === 0 || isLoading} className="group rounded-full shadow-sm">
      ...
      <div className="w-80 border-l border-border p-6 bg-secondary/20">
        ...
        <div className="space-y-4">
          {quickActions.map((action, index) => <Card key={index} className="cursor-pointer hover:shadow-medium transition-smooth" onClick={action.action}>
